...
class SandboxWaitTimeoutError(TimeoutError):
    def __init__(
        self,
        *,
        sandbox_id: str,
        timeout_sec: float,
        last_sandbox: Optional[Sandbox] = None,
    ) -> None:
        super().__init__(f"timed out waiting for sandbox {sandbox_id} after {timeout_sec} seconds")
...
class TemplateWaitTimeoutError(TimeoutError):
    def __init__(
        ...
        *,
        ...
        super().__init__(f"timed out waiting for template {template_id} after {timeout_sec} seconds")
...
class TemplateCreationFailedError(RuntimeError):
    def __init__(self, template: Template) -> None:
        creation = None
        if not isinstance(template.status, Unset) and not isinstance(template.status.creation, Unset):
