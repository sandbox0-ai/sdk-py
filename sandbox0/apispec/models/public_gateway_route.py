...
T = TypeVar("T", bound="PublicGatewayRoute")


@_attrs_define
class PublicGatewayRoute:
    """
    Attributes:
        id (str): Stable route id. Must be a DNS label and unique within the sandbox policy.
        port (int):
        resume (bool): Route-level resume gate for public gateway traffic. Evaluated only when
            sandbox auto_resume is true.
        path_prefix (Union[Unset, str]): Request path prefix matched before proxying to the sandbox port. Default: '/'.
        methods (Union[Unset, list[str]]): Allowed HTTP methods. Empty allows every method.
        rewrite_prefix (Union[None, Unset, str]): Optional replacement prefix applied before proxying.
        auth (Union[Unset, PublicGatewayAuth]):
        cors (Union[Unset, PublicGatewayCORS]):
        rate_limit (Union[Unset, PublicGatewayRateLimit]):
        timeout_seconds (Union[Unset, int]): Per-route upstream timeout in seconds. Zero disables the route override.
    """
    ...
    def from_dict(cls: type[T], src_dict: Mapping[str, Any]) -> T:
        ...
        public_gateway_route.additional_properties = d
        return public_gateway_route
